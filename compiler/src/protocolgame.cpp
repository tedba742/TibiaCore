...
bool ProtocolGame::stopLiveCast()
{
	...
	deleteLiveInfoPlayer(player->getName());
	unregisterLiveCast();
	return true;
}
...
void ProtocolGame::addSpectator(ProtocolSpectator_ptr spectatorClient)
{
	...
	entry << "Spectator[" << spectatorRecord << "]";
	spectInfo.push_back({ getLiveCastName(), name = entry.str(), false, false, convertIPToString(getIP()) });
	sendToChannel(nullptr, TALKTYPE_CHANNEL_O, entry.str() + " watching now.", CHANNEL_CAST);
}

void ProtocolGame::removeSpectator(ProtocolSpectator_ptr spectatorClient)
{
	sendToChannel(nullptr, TALKTYPE_CHANNEL_O, name + " left.", CHANNEL_CAST);
	std::lock_guard<decltype(liveCastLock)> lock(liveCastLock);
	//DO NOT do any send operations here
...
void ProtocolGame::spectatelogout()
{
	...
	if (spect && player) {
		deleteLiveInfoSpect(spect->getLiveCastName(), spect->name, convertIPToString(spect->getIP()));
		spect->removeSpectator(std::static_pointer_cast<ProtocolSpectator>(shared_from_this()));
		player->decrementReferenceCounter();
		player = nullptr;
	}
...
void ProtocolGame::spectate(const std::string& liveCastName, const std::string& password)
{
	...
	if (!foundPlayer || foundPlayer->isRemoved()) {
		disconnectClient("Live stream off-line. Please relogin to refresh the list.");
		return;
	}
	...
	if (liveCasterProtocol->isLiveCaster()) {
		if (!liveCastPassword.empty() && password != liveCastPassword) {
			disconnectClient("This live stream is protected with password.");
			return;
		}
		...
		if (spectInfo.size() > 0) {
			for (auto& info : spectInfo) {
				if (info.liveName == spect->getLiveCastName() && info.spectIp == convertIPToString(spect->getIP())) {
					if (info.spectBan) {
						disconnectClient("You are banned from this live!");
						return;
					}
					//info.spectIp.size() > 1 && 
					/*if (!info.spectMute && info.spectIp == convertIPToString(spect->getIP())) {
						disconnectClient("You are already logging into this live!");
						return;
					}*/
				}
			}
		...
		sendChannel(CHANNEL_CAST, LIVE_CAST_CHAT_NAME);
		// Dont use with classic client.
		std::ostringstream ss;
		ss << ">>>Welcome the live<<<\nCommand: /name (new spectator name)";
		sendTextMessage(TextMessage(MESSAGE_EVENT_ADVANCE, ss.str()));
	}
	else {
		disconnectClient("Live stream off-line. Please relogin to refresh the list.");
	}
...
void ProtocolGame::deleteLiveInfoPlayer(const std::string& playerName) {

	auto it = std::remove_if(spectInfo.begin(), spectInfo.end(),
		[&playerName](const LiveInfo& info) {
			return info.liveName == playerName;
		});
	spectInfo.erase(it, spectInfo.end());
}

void ProtocolGame::deleteLiveInfoSpect(const std::string& liveName, const std::string& spectName, const std::string& spectIp) {

	for (auto& info : spectInfo) {
		if (info.liveName == liveName && info.spectName == spectName && info.spectIp == spectIp && (info.spectBan || info.spectMute)) {
			return;
		}
	}
	auto it = std::remove_if(spectInfo.begin(), spectInfo.end(),
		[&liveName, &spectName, &spectIp](const LiveInfo& info) {
			return info.liveName == liveName && info.spectName == spectName && info.spectIp == spectIp;
		});
	spectInfo.erase(it, spectInfo.end());
}
...
void ProtocolGame::parseSpectatorSay(NetworkMessage& msg)
{
	...
	if (m_spectator)
	{
		...
		if (text.substr(0, 5) == "/name" && text.length() > 6) {
			...
			if (newName.length() < 4) {
				sendToChannel(nullptr, TALKTYPE_CHANNEL_R1, "This name have less than 4 letters.", CHANNEL_CAST);
				return;
			}
			if (newName.length() > 20) {
				sendToChannel(nullptr, TALKTYPE_CHANNEL_R1, "This name have more than 20 letters.", CHANNEL_CAST);
				return;
			}
			if (iSpaces > 1) {
				sendToChannel(nullptr, TALKTYPE_CHANNEL_R1, "This name have more than one whitespace.", CHANNEL_CAST);
				return;
			}

			bool nameCheck = false;
			for (auto& info : spectInfo) {
				if (info.spectName != newName && info.liveName == spect->getLiveCastName()) {
					nameCheck = true;
					if (info.spectIp == convertIPToString(spect->getIP())) {
						spect->name = info.spectName = newName;
						sendToChannel(nullptr, TALKTYPE_CHANNEL_R1, "Your spectator name: " + newName, CHANNEL_CAST);
						break;
					}
				}
			}
			if (!nameCheck) {
				sendToChannel(nullptr, TALKTYPE_CHANNEL_R1, "This name already exists!", CHANNEL_CAST);
				return;
			}
			return;
		}
		if (OTSYS_TIME() < (lastTalkTime + 5000)) {
			sendToChannel(nullptr, TALKTYPE_CHANNEL_R1, "You should wait a while to speak again.", CHANNEL_CAST);
			return;
		}

		for (auto& info : spectInfo) {
			if (/*info.spectName == spect->name && */ info.spectIp == convertIPToString(spect->getIP()) && info.spectMute) {
				sendToChannel(nullptr, TALKTYPE_CHANNEL_R1, "You are muted!", CHANNEL_CAST);
				return;
			}
		}
		...
		if (spect) {
			std::ostringstream entry;
			entry << spect->name << ": " << text;
			player->sendToChannel(nullptr, TALKTYPE_CHANNEL_Y, entry.str(), CHANNEL_CAST);
		}
...
void ProtocolGame::parsePacket(NetworkMessage& msg)
{
	...
	//std::cout << "Recvbyte Player: 0x" << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(recvbyte) << std::endl;
...
void ProtocolGame::parseSay(NetworkMessage& msg)
{
	...
	if (channelId == CHANNEL_CAST)
	{
		std::ostringstream entry;
		if (text.find("/mute ") == 0) {
			std::string spectatorName = text.substr(6);

			bool found = false;
			for (auto& info : spectInfo) {
				if (info.spectName == spectatorName && info.liveName == getLiveCastName()) {
					info.spectMute = true;
					found = true;
					entry << "Spectator " << info.spectName << " muted.";
					break;
				}
			}
			if (!found) {
				entry << "Spectator " << spectatorName << " not found.";
			}
			sendToChannel(nullptr, TALKTYPE_CHANNEL_O, entry.str(), CHANNEL_CAST);
			return;
		}
		else if (text.find("/ban ") == 0) {
			std::string spectatorName = text.substr(5);
			...
			for (auto& info : spectInfo) {
				if (info.spectName == spectatorName && info.liveName == getLiveCastName()) {
					info.spectBan = true;
					found = true;
					entry << "Spectator " << info.spectName << " banished.";
					break;
				}
			}
			if (!found) {
		...
		sendToChannel(player, TALKTYPE_CHANNEL_O, entry.str(), CHANNEL_CAST);
		return;
	}
...
void ProtocolGame::sendAddCreature(const Creature* creature, const Position& pos, int32_t stackpos, bool isLogin)
{
	...
	msg.add<uint16_t>(0x32); // best duration (50)
