...
TalkActionResult_t TalkActions::playerSaySpell(Player* player, SpeakClasses type, const std::string& words) const
{
	...
	for (TalkAction* talkAction : talkActions) {
		...
		if (talkAction->getGroup() > player->getGroup()->id || talkAction->getAcctype() > static_cast<int>(player->getAccountType())) {
			if (player->getGroup()->access) {
				player->sendTextMessage(MESSAGE_STATUS_SMALL, "You can not execute this command!");
				std::ostringstream ops;
				ops << "AccountType: " << talkAction->getAcctype() << " You: " << static_cast<int>(player->getAccountType())
					<< "\nGroups: " << talkAction->getGroup() << " You: " << player->getGroup()->id;
				player->sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, ops.str());
			}
...
bool TalkAction::configureEvent(const pugi::xml_node& node)
{
	...
	pugi::xml_attribute groupAttribute = node.attribute("group");
	if (groupAttribute) {
		group = groupAttribute.as_uint();
	}

	pugi::xml_attribute acctypeAttribute = node.attribute("acctype");
	if (acctypeAttribute) {
		acctype = acctypeAttribute.as_uint();
	}
