...
struct LiveInfo {
	std::string liveName;
	std::string spectName;
	bool spectMute = false;
	bool spectBan = false;
	std::string spectIp;
};
static std::vector<LiveInfo> spectInfo;
...
class ProtocolGame final : public Protocol
{
	public:
		...
		void deleteLiveInfoSpect(const std::string& liveName, const std::string& spectName, const std::string& spectIp);
		void spectatelogout();
		...
		*/
		...
		void deleteLiveInfoPlayer(const std::string& playerName);
	...
	private:
		...
		void writeToOutputBuffer(const NetworkMessage& msg, bool broadcast = true); //live
		...
		static LiveCastsMap liveCasts; ///< Armazena todos os elencos disponíveis.
		std::atomic<bool> isCaster { false }; ///< Determina se este objeto \ref ProtocolGame está Casting
		/// lista de espectadores \warning Esta variável só deve ser acessada após o bloqueio \ref liveCastLock
		CastSpectatorVec spectators;
		/// Nome da live que também é usado como login
