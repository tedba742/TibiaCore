...
void MonsterType::createLoot(Container* corpse)
{
	...
	if (g_config.getBoolean(ConfigManager::SHOW_MONSTER_LOOT)) {
		...
		if (owner && owner->isPremium()) {
			...
			ss << "Loot of " << name << ": " << corpse->getContentDescription();
...
std::vector<Item*> MonsterType::createLootItem(const LootBlock& lootBlock, Container* corpse)
{
	...
	if (owner) {
		...
		// Luck type and msg drop very rare item with luck.
		...
		if (chance < 5 && itemCount > 0 && lucklootEq) {
			...
			itemCount > 1 ? sss << "Lucky: " << owner->getName() << " droped very rare " << itemCount << "x " << Item::items[lootBlock.id].pluralName << " from " << nameDescription << "." :
				sss << "Lucky: " << owner->getName() << " droped very rare " << itemCount << "x " << Item::items[lootBlock.id].name << " from " << nameDescription << ".";
		...
		if (randvalue < rate * chance) {
			// Default Loot drop monster owner.
			if (Item::items[lootBlock.id].stackable) {
				itemCount = randvalue % countMax;
			}
			else {
